...
{
    /**
     * Array of the different opcodes and how they are mapped to HTML classes.
     *
     * @var array<int,string>
     */
    public const OP_CLASS_MAP = [
    ...
    {
        $rendered = $this->renderChanges($this->getChanges($differ));
    ...
    {
        $this->ensureChangesUseIntOp($differArray);

        $rendered = $this->renderChanges($differArray);
     ...
     */
    abstract protected function renderChanges(array $changes): string;
    ...
    final protected function formatChanges(array &$changes): void
    {
        foreach ($changes as &$hunk) {
            foreach ($hunk as &$block) {
                ...
                if ($block['op'] & (SequenceMatcher::OP_REP | SequenceMatcher::OP_DEL)) {
                ...
                /** @phan-suppress-next-line PhanTypeInvalidLeftOperandOfBitwiseOp */
                if ($block['op'] & (SequenceMatcher::OP_REP | SequenceMatcher::OP_INS)) {
    ...
    /**
     * Make sure the "changes" array uses int "op".
     *
     * Internally, we would like always int form for better performance.
     *
     * @param array[][] $changes the changes
     */
    protected function ensureChangesUseIntOp(array &$changes): void
    {
        // check if the op is already int type
        if (\is_int($changes[0][0]['op'] ?? null)) {
            return;
        }

        foreach ($changes as &$hunks) {
            foreach ($hunks as &$block) {
                $block['op'] = SequenceMatcher::opStrToInt($block['op']);
            }
        }
    }
