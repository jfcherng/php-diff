...
namespace Jfcherng\Diff\Test\Contract;
...
final class RendererTest extends TestCase
{
     ...
     *
     * @covers \Jfcherng\Diff\Contract\Renderer\AbstractRenderer::setOptions
     */
     ...
     *
     * @covers \Jfcherng\Diff\Contract\Renderer\AbstractRenderer::renderArray
     */
    public function testHtmlRendererRenderWithResultFromJsonRenderer(): void
    {
        ...
        foreach ($rendererNames as $rendererName) {
            ...
            // test "outputOpAsString" is false
            $jsonResult = DiffHelper::calculate(
                $old,
                $new,
                'JsonHtml',
                $differOptions,
                ['outputOpAsString' => false] + $rendererOptions,
            );

            static::assertSame(
                $goldenResult,
                $renderer->renderArray(json_decode($jsonResult, true)),
                "HTML renderers should be able to render with JSON result. ('outputOpAsString' => false)",
            );

            // test "outputOpAsString" is true
            $jsonResult = DiffHelper::calculate(
                $old,
                $new,
                'JsonHtml',
                $differOptions,
                ['outputOpAsString' => true] + $rendererOptions,
            );

            static::assertSame(
                $goldenResult,
                $renderer->renderArray(json_decode($jsonResult, true)),
                "HTML renderers should be able to render with JSON result. ('outputOpAsString' => true)",
            );
        }
    }
